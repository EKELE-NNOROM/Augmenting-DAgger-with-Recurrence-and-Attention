...
def run(steering_network, args):
    ...
    env.close()
    print(sum(cumulative_reward))
    return sum(cumulative_reward)
...
if __name__ == "__main__":
    ...
    # steering_network = OriginalDrivingPolicy(n_classes=args.n_steering_classes).to(DEVICE)
    # steering_network = OriginalDrivingPolicyDropOut(n_classes=args.n_steering_classes).to(DEVICE)
    # steering_network = RecurrentNetwork(n_classes=args.n_steering_classes).to(DEVICE)
    # steering_network = RecurrentNetworkDropOut(n_classes=args.n_steering_classes).to(DEVICE)
    # steering_network = AttentionNetwork(n_classes=args.n_steering_classes).to(DEVICE)
    # steering_network = AttentionNetworkDropOut(n_classes=args.n_steering_classes).to(DEVICE)
    steering_network = RecurrentAttention(n_classes=args.n_steering_classes).to(DEVICE)
    # steering_network = RecurrentAttentionDropOut(n_classes=args.n_steering_classes).to(DEVICE)
    if args.learner_weights:
        steering_network.load_weights_from(args.learner_weights)

    run(steering_network, args)
    #print(run(steering_network, args))
